...
export function activate(context: vscode.ExtensionContext) {
 ...
 const error_color =
  vscode.workspace.getConfiguration("indentColorizer")["errorColor"] ||
  "rgba(128,32,32,0.3)";
 ...
 const tabmix_color =
  vscode.workspace.getConfiguration("indentColorizer")["tabmixColor"] || "";
 ...
 const ignoreLinePatterns =
  vscode.workspace.getConfiguration("indentColorizer")["ignoreLinePatterns"] ||
  [];
 const colorOnWhiteSpaceOnly =
  vscode.workspace.getConfiguration("indentColorizer")[
   "colorOnWhiteSpaceOnly"
  ] || false;

 // Colors will cycle through, and can be any size that you want
 const colors = vscode.workspace.getConfiguration("indentColorizer")[
  "colors"
 ] || [
 ...
 function indentConfig() {
  var skiplang =
   vscode.workspace.getConfiguration("indentRainbow")["ignoreErrorLanguages"] ||
   [];
  skipAllErrors = false;
 ...
 function checkLanguage() {
  if (activeEditor) {
   if (currentLanguageId !== activeEditor.document.languageId) {
    var inclang =
     vscode.workspace.getConfiguration("indentRainbow")["includedLanguages"] ||
     [];
    var exclang =
     vscode.workspace.getConfiguration("indentRainbow")["excludedLanguages"] ||
     [];
